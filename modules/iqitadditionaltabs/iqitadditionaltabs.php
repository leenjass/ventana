...
class IqitAdditionalTabs extends Module implements WidgetInterface
{
    ...
    public function addFaqAccordians($description) {
        
		preg_match_all('/\{pre_cms_accordion\:[(0-9\,)]+\}/i', $description, $matches);
        foreach ($matches[0] as $index => $match) {
            $explode = explode(":", $match);

            $description = str_replace($match, $this->returnPreCmsAccordionContent(str_replace("}", "", $explode[1])), $description);
        }
    ...
    protected function returnPreCmsAccordionContent($id_precmsaccordions)
    {
        ...
        $content = $this->fetch('module:iqitadditionaltabs/views/templates/hook/hookFilterCmsContent.tpl');
        return $content;
    }
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        if (preg_match('/^displayAboveProductsTabs\d*$/', $hookName)) {
            $idProduct = (int) Tools::getValue('id_product');
            $tabs = IqitAdditionalTab::getTabs('all', $idProduct, true);
            foreach ($tabs as $key => $tab) {
                if ($tab['title'] && $tab['description']) {         
                    $tabs[$key]['description'] = $this->addFaqAccordians($tab['description']);
                }
            }
