					...
					{if $product['customizedDatas']}
						<tr>
							...
							<td><a href="{$link->getAdminLink('AdminProducts', true, ['id_product' => $product.id_product, 'updateproduct' => 1])|escape:'html':'UTF-8'}">
								...
								{hook h='displayProductOptionCart' id_cart=$cart->id no_view=0 pos=$item is_native=1}
							</td>
            ...
            {foreach $product['customizedDatas'] as $customizationPerAddress}
              {foreach $customizationPerAddress as $customization}
						    ...
						    <tr>
							    ...
							    <td></td>
							    <td>{hook h='displayProductOptionCart' id_cart=$cart->id no_view =1 pos=$item is_native=1}</td>
						    </tr>
              {/foreach}
						{/foreach}
					{/if}
		...
		<table class="table">
			<thead>
				<tr>
					...
					<th>{l s='Discount name' d='Admin.Global'}</th>
