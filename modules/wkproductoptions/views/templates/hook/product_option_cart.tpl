...
{block name='cart_detailed_product_line'}
    ...
    <div class="product-line-grid-left col-md-3 col-xs-4">
        ...
        {if $product.default_image}
            <img src="{$product.default_image.bySize.cart_default.url}" alt="{$product.name|escape:'quotes':'UTF-8'}" loading="lazy">
        {else}
    ...
    <div class="product-line-grid-body col-md-4 col-xs-8">
        ...
        <div class="product-line-info {$attribute|lower|escape:'html':'UTF-8'}">
            <span class="label">{$attribute}:</span>
            <span class="value">{$value}</span>
        </div>
        {/foreach}
        {*Start- custom hook for product options*}
        {hook h='displayAfterCartProductLine' product=$product type=1}
        {*End- custom hook for product options*}
        {if is_array($product.customizations) && $product.customizations|count}
        <br>
        {if WkProductCustomerOptions::checkOrderContainsOptionBeforeOrder(
    ...
    {*Start- custom hook for product options*}
    {hook h='displayAfterCartProductLine' product=$product type=2}
    {*End- custom hook for product options*}
    </div>
{/block}
