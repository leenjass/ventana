...
class HookOrder extends AbstractHook
{
    ...
    {
        try {
            $idOrder = empty($params['order']) ? $params['objOrder']->id : $params['order']->id;
            ...
            $confirmationDetails = $hookService->getOrderConfirmationDetails(
                Context::getContext()->shop->id,
                Context::getContext()->language->id,
                (int) $idOrder
            );

            if (empty($confirmationDetails)) {
                return;
            }
            ...
            return $this->module->display(
                $file,
                'views/templates/front/order.tpl'
            );
        } catch (\Exception $e) {
            return;
        }
    ...
    public function actionOrderStatusPostUpdate($params)
    {
        ...
        $idOrderStatus = $params['newOrderStatus']->id;

        try {
            /** @var OrderStatusService $orderStatusService */
            $orderStatusService = ServiceContainer::getInstance()->get(OrderStatusService::class);

            return $orderStatusService->sendOrderStatusEvent(new Order($idOrder), $idOrderStatus);
        } catch (Exception $e) {
            return false;
        }
    }
