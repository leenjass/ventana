...
class ChannelRepository
{
     ...
     */
    public function getChannelsByClientId($clientId)
    {
        $query = new DbQuery();
        $query->select(TrustedshopsChannel::$definition['primary'])
            ->from(TrustedshopsChannel::$definition['table'])
            ->where('id_client = \'' . pSQL($clientId) . '\'');
    ...
    public function getChannelsByChannelId($clientId, $channelId, $idLang = null, $idShop = null)
    {
        ...
        $query->select(TrustedshopsChannel::$definition['primary'])
            ->from(TrustedshopsChannel::$definition['table'])
            ->where('id_client = \'' . pSQL($clientId) . '\'')
            ->where('e_trusted_channel_ref = \'' . pSQL($channelId) . '\'');

        if (!is_null($idLang)) {
            $query->where('id_lang = ' . (int) $idLang);
        }
    ...
    public function clearChannels()
    {
        return Db::getInstance()->delete(
            TrustedshopsChannel::$definition['table'],
            '1'
        );
    }
}
