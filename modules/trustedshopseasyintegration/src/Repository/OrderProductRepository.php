...
{
    public function getOrderDetails($idShop, $idLang, $nbDays, $withProductData = true)
    {
        $query = new DbQuery();
        $query->select('DISTINCT o.reference, o.date_add AS transaction_date, c.email, c.firstname, c.lastname'
            . ($withProductData ? ', od.product_id' : '')
        );
        $query->from('order_detail', 'od');
        $query->innerJoin('orders', 'o', 'o.id_order = od.id_order');
        $query->innerJoin('customer', 'c', 'o.id_customer = c.id_customer');
        $query->where('o.`id_lang` = ' . (int) $idLang);
        $query->where('o.`id_shop` = ' . (int) $idShop);

        if ($nbDays > 0) {
            $compareDate = date('Y-m-d 00:00:00', strtotime("-{$nbDays}days", time()));
            $query->where('o.`date_add` >= \'' . pSQL($compareDate) . '\'');
        }
