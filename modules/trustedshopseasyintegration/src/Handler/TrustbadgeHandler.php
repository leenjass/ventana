...
class TrustbadgeHandler
{
    ...
    public function handle($config)
    {
        ...
        foreach ($config['children'] as $child) {
            ...
            $trustbadgeModel = new TrustbadgeFooterModel();
            $trustbadgeModel->setTag($child['tag']);

            foreach ($child['attributes'] as $attribute) {
                if (!isset($attribute['value']) || !isset($attribute['attributeName'])) {
                    continue;
                }
                $trustbadgeModel->add($attribute['attributeName'], $attribute['value']);
            }

            $models[] = $trustbadgeModel;
        }
