...
use TrustedshopsAddon\Model\Widget\WidgetScriptModel;

class WidgetScriptHandler
{
    public function handle($config)
    {
        ...
        foreach ($config['children'] as $child) {
            ...
            $widgetScriptModel = new WidgetScriptModel();
            $widgetScriptModel->setTag($child['tag']);
            foreach ($child['attributes'] as $attribute) {
                if (!isset($attribute['attributeName'])) {
                    continue;
                }
                $widgetScriptModel->add(
                    $attribute['attributeName'],
                    isset($attribute['value']) ? $attribute['value'] : ''
                );
            }

            $models[] = $widgetScriptModel;
        }
