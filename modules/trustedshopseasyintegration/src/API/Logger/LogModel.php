...
use TrustedshopsAddon\API\Model\EventResponse;
...
class LogModel
{
    ...
    {
        return $this->configuredTriggerStatus;
    }
     ...
     * @return LogModel
     */
    public function setConfiguredTriggerStatus($configuredTriggerStatus)
    {
        $this->configuredTriggerStatus = $configuredTriggerStatus;
    ...
    public function toArray()
    {
        $data = [
            ...
            'new_order_status' => $this->getNewOrderStatus(),
            'trigger_status' => $this->getConfiguredTriggerStatus(),
        ];

        if (!empty($this->getResponse())) {
            $data['response_code'] = $this->getResponse()->getStatusCode();
            if ($this->getResponse()->getModel() instanceof EventResponse) {
                $data['event_reference'] = $this->getResponse()->getModel()->getEventRef();
            }
        }
