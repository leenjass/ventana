...
class EventsResponse extends AbstractResponse
{
    public function getModel()
    {
        $content = (string) $this->body;
        ApiLogger::getInstance()->logInfo('Event response : ' . $content);
        ...
        if ($this->getStatusCode() < 200 || $this->getStatusCode() > 299) {
            ApiLogger::getInstance()->logInfo('Error in event response : ' . json_encode($output));

            return (new Error())->hydrate($output);
        }

        return (new EventResponse())->hydrate($output);
    }
}
