...
{
    public function saveCredentials(CredentialsModel $credentialsModel)
    {
        Configuration::updateGlobalValue(\Trustedshopseasyintegration::CLIENT_ID, $credentialsModel->getClientId());
        Configuration::updateGlobalValue(\Trustedshopseasyintegration::CLIENT_SECRET, $credentialsModel->getClientSecret());
    }

    public function getCredentials()
    {
        $clientId = Configuration::get(\Trustedshopseasyintegration::CLIENT_ID);
        $clientSecret = Configuration::get(\Trustedshopseasyintegration::CLIENT_SECRET);

        $this->checkCredentials($clientId, $clientSecret);

        return (new CredentialsModel())
            ->setClientId(!empty($clientId) ? $clientId : '')
            ->setClientSecret(!empty($clientSecret) ? $clientSecret : '');
    }

    private function checkCredentials($clientId, $clientSecret)
    {
        $loginUpdated = false;
        ...
        if (trim($clientSecret) !== $clientSecret) {
