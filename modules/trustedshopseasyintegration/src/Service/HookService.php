...
use TrustedshopsAddon\Handler\WidgetScriptHandler;
...
class HookService
{
     ...
     */
    public function getTrustbadge($idShop, $idLang)
    {
        $channel = $this->channelService->getChannelFromIdShopIdLang($idShop, $idLang);

        if (empty($channel) || empty($channel->id_trustbadge) || empty($channel->trustbadge_config)) {
            return null;
        }

        $trustbadgeHandler = new TrustbadgeHandler();

        return $trustbadgeHandler->handle($channel->trustbadge_config);
    }

    public function getWidget($idShop, $idLang, $location, $params)
    {
        $channel = $this->channelService->getChannelFromIdShopIdLang($idShop, $idLang);

        if (empty($channel) || empty($channel->widget_config)) {
            return null;
        }

        $idProduct = empty($params['id_product']) ? null : $params['id_product'];

        return (new WidgetHandler())
            ->setIdLang($idLang)
            ->setIdShop($idShop)
            ->handle($channel->widget_config, $location, $idProduct);
    }

    public function getWidgetScriptLink($idShop, $idLang)
    {
        $channel = $this->channelService->getChannelFromIdShopIdLang($idShop, $idLang);
        ...
        $widgetScriptHandler = new WidgetScriptHandler();

        return $widgetScriptHandler->handle($channel->widget_config);
    }
     ...
     * @throws \PrestaShopException
     */
    public function getOrderConfirmationDetails($idShop, $idLang, $idOrder)
    {
        $channel = $this->channelService->getChannelFromIdShopIdLang($idShop, $idLang);
        ...
        $orderHandler = new OrderConfirmationHandler();
