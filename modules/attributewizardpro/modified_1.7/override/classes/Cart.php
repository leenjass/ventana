...
class Cart extends CartCore
{
        ...
        foreach ($result as &$row) {
            ...
            if (!$row['is_gift'] || (int) $row['cart_quantity'] === $givenAwayQuantity) {
                ...
                $row = $this->applyProductCalculations(
                    $row, $cart_shop_context, $row['cart_quantity'] - $givenAwayQuantity
                );
            }
    ...
    protected function applyProductCalculations($row, $shopContext, $productQuantity = null)
    {
        ...
        if (version_compare(_PS_VERSION_,  '1.7.7.0', '<') || Context::getContext()->controller->controller_type == 'front')
        {
            return parent::applyProductCalculations($row, $shopContext, $productQuantity);
        }
